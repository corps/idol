...
import { asCodegenOutput, asScaffoldOutput, asTypedGeneratorOutput, build, GeneratorConfig as BaseGeneratorConfig, materialTypeMapper, render, scalarMapper, SinglePassGeneratorOutput, TypedOutputBuilder, typeMapper, typeStructMapper, withCommentHeader } from "./generators";
import type { GeneratorParams, MaterialTypeHandler, OutputTypeSpecifier, ScalarHandler, ScalarMapper, Tags, TypeHandler, TypeMapper, TypeStructHandler, TypeStructMapper } from "./generators";
...
export class CodegenTypeStructExpressionHandler implements TypeStructHandler<scripter.Stringable> {
    ...
    get Repeated() {
        return (scalar: scripter.Stringable, tags?: Tags = {}) => scripter.newInvocation("GraphQLList_", scalar);
    }
...
export class CodegenTypeHandler implements TypeHandler<TypedOutputBuilder, scripter.Stringable> {
    ...
    get Enum() {
        return (type: Type, options: string[]) => new TypedOutputBuilder([
            scripter.exportedConst(asQualifiedIdent(type.named), this.enumConst(options)),
        ].concat(type.named.qualifiedName in this.params.scaffoldTypes.obj ? [] : [
            scripter.exportedConst(asGraphQLTypeIdent(type.named),
                scripter.newInvocation("GraphQLEnumType_", scripter.invocation("wrapValues_", asQualifiedIdent(type.named)))),
        ]), {
            imports: this.config.typeImports({ codegen: type.named.qualifiedName })(type)
        });
    }
    ...
    get Field() {
        ...
        return (typeStruct: TypeStruct, tags?: Tags = {}) => {
            ...
            return scripter.objLiteral(
                scripter.propDec("description", scripter.literal(getTagValue(tags.fieldTags, "description", ""))),
    ...
    get Struct() {
        return (type: Type, fields: OrderedObj<scripter.Stringable>) => new TypedOutputBuilder([
            ...
            scripter.exportedConst(asGraphQLTypeIdent(type.named), scripter.newInvocation("GraphQLObjectType_", this.objectTypeLiteral(type))),
        ]), {
            imports: this.config.typeImports({ codegen: type.named.qualifiedName })(type),
        });
    }
...
export class GeneratorConfig extends BaseGeneratorConfig {
    ...
    fieldsImport(from: OutputTypeSpecifier<>, reference: Reference): OrderedObj<StringSet> {
        return new OrderedObj<StringSet>({
            [this.resolvePath(from, { codegen: reference.qualifiedName })]: new StringSet([asGraphQLFieldsIdent(reference)])
        })
    }

    scaffoldTypeImport(from: OutputTypeSpecifier<>, reference: Reference): OrderedObj<StringSet> {
        return new OrderedObj<StringSet>({
            [this.resolvePath(from, { scaffold: reference.qualifiedName })]: new StringSet([`${asGraphQLTypeIdent(reference, false)} as ${asGraphQLTypeIdent(reference)}`])
        })
    }

    codegenTypeImport(from: OutputTypeSpecifier<>, reference: Reference): OrderedObj<StringSet> {
        return new OrderedObj<StringSet>({
            [this.resolvePath(from, { codegen: reference.qualifiedName })]: new StringSet([asGraphQLTypeIdent(reference)]),
        })
    }
    ...
    scalarImports(module: OutputTypeSpecifier<>): ScalarMapper<OrderedObj<StringSet>> {
        return scalarMapper<OrderedObj<StringSet>>({
            Literal: (primitiveType: string, value: any) => this.graphQLImportOfPrimitive(module, primitiveType),
    ...
    typeStructImports(module: OutputTypeSpecifier<>): TypeStructMapper<OrderedObj<StringSet>> {
        return typeStructMapper({
            ...
            Repeated: (scalarImports: OrderedObj<StringSet>) => scalarImports.concat(this.graphQLImports(module, "GraphQLList")),
    ...
    typeImports(module: OutputTypeSpecifier<>): TypeMapper<OrderedObj<StringSet>> {
        ...
        return typeMapper<OrderedObj<StringSet>, OrderedObj<StringSet>>({
            ...
            Enum: (...args) => this.graphQLImports(module, "GraphQLEnumType").concat(this.idolGraphQLImports(module, "wrapValues")),
            Struct: (type: Type, fieldImports: OrderedObj<OrderedObj<StringSet>>) =>
                this.graphQLImports(module, "GraphQLObjectType").concat(flattenOrderedObj(fieldImports, new OrderedObj()))
        })
    }
}
...
export class ScaffoldTypeHandler implements MaterialTypeHandler<TypedOutputBuilder> {
    ...
    get Enum() {
        return (type: Type) => new TypedOutputBuilder([
            ...
            scripter.exportedConst(type.named.typeName, scripter.objLiteral("..." + asQualifiedIdent(type.named))),
            // And then the type
            scripter.exportedConst(asGraphQLTypeIdent(type.named),
                scripter.newInvocation("GraphQLEnumType_", scripter.invocation("wrapValues_", type.named.typeName))),
        ], {
            imports: this.config.codegenEnumImport({ scaffold: type.named.qualifiedName }, type.named).concat(
                this.config.idolGraphQLImports({ scaffold: type.named.qualifiedName }, "wrapValues")).concat(
    ...
    get Struct() {
        return (type: Type) => new TypedOutputBuilder([
            scripter.exportedConst(asGraphQLTypeIdent(type.named, false), scripter.newInvocation("GraphQLObjectType_",
                scripter.objLiteral(
                    scripter.propDec("fields", scripter.objLiteral(`...${asGraphQLFieldsIdent(type.named)}`)),
                    scripter.propDec("name", scripter.literal(type.named.typeName)),
                    scripter.propDec("description", scripter.literal(type.getTagValue("description", ""))),
                ),
            ))
        ], {
            imports: this.config.fieldsImport({ scaffold: type.named.qualifiedName }, type.named).concat(
                this.config.graphQLImports({ scaffold: type.named.qualifiedName }, "GraphQLObjectType")),
        });
    }
}
...
export const idolGraphQLJsOutput = (config: GeneratorConfig) => new SinglePassGeneratorOutput({
    supplemental: new OrderedObj<Conflictable<string>>({
        [config.idolGraphqlPath]:
            new Conflictable([idolGraphQlJs]),
    }),
});
...
function main() {
    const params = start({
        ...
        args: {
            "target":
                "idol module names whose contents will have extensible GraphQLType's scaffolded.",
            "output":
