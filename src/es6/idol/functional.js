...
export class OrderedObj<I> {
    ...
    mapAndFilter<R>(f: I => Alt<R>): OrderedObj<R> {
        return OrderedObj.fromIterable<R>(
            this.keys().reduce((result, k) => result.concat(f(this.obj[k]).map(v => [new OrderedObj<R>({ [k]: v })]).getOr([])), [])
        )
    }
    ...
    concatMap<R: { concat(r: any): any } | Array<any> | any[]>(f: (k: string, i: I) => R, d: R): R {
        return this.ordering.reduce((result: R, nextK: string): R => result.concat(f(nextK, this.obj[nextK])), d);
    }
