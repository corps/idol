...
export function prepareGeneratorParams(options: { [k: string]: Array<string> }, data: string): GeneratorParams {
    ...
    const allTypes: OrderedObj<Type> = concatMap(allModules, ([module, _]: [Module, string]) => moduleTypesAsOrderedObj(module),
        new OrderedObj());

    const targets: Array<string> = options.target || [];
    const scaffoldTypes = flatten<OrderedObj<Type>, OrderedObj<Type>>(targets.map(target => {
