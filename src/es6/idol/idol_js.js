...
export class CodegenTypeHandler implements TypeHandler<TypedOutputBuilder, scripter.Stringable> {
  ...
  structClass(type: Type, fields: OrderedObj<scripter.Stringable>) {
    return scripter.exportedClass(
      this.exportIdent(type),
      null,
      [
        scripter.methodDec("constructor", ["val"], [scripter.assignment("this._original", "val")]),
        scripter.staticMethodDec("validate", ["val"], []),
        scripter.staticMethodDec("isValid", ["val"], [scripter.ret(scripter.literal(true))]),
        scripter.staticMethodDec("expand", ["val"], [scripter.ret("val")]),
        scripter.staticMethodDec(
          "wrap",
          ["val"],
          [scripter.ret(scripter.newInvocation(this.exportIdent(type), "val"))]
        ),
        scripter.staticMethodDec("unwrap", ["val"], [scripter.ret("val")])
      ].concat(
        concatMap(
          fields.ordering,
          fieldName => {
            const originalField = this.gettersAndSettersFor(
              fieldName,
              fieldName,
            ...
            const camelField =
              fieldName !== camelCase(fieldName)
                ? this.gettersAndSettersFor(camelCase(fieldName), fieldName, fields.obj[fieldName])
                : [];
