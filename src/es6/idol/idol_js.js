...
export class IdolJsCodegenStruct extends GeneratorFileContext<IdolJs> {
    ...
    get declaredIdent(): Alt<Exported> {
        return cachedProperty(this, "declaredIdent", () => {
            const fieldConstructorIdents: OrderedObj<string> = this.fields.mapAndFilter(codegenTypeStruct => codegenTypeStruct.constructorExpr).map(expr => expr(this.state, this.path));
                    ...
                    (ident: string) => [
                        scripter.classDec([
                            ...
                            ...fieldConstructorIdents.concatMap<Array<string>>((fieldName, constructor) => {
                                ...
                                const fields = this.gettersAndSettersFor(
                                    fieldName,
                                    fieldName,
                                    constructor,
                                );

                                return fieldName === camelFieldName ? fields : fields.concat(
                            ...
                            scripter.arrayLiteral(
                                ...fieldConstructorIdents.mapIntoIterable((fieldName, constructor) =>
                                    scripter.objLiteral(
                                        ...
                                        scripter.propDec(
                                            "optional",
                                            scripter.literal(this.fields.obj[fieldName].tsDecon.context.includesTag("optional"))
                                        )
                                    ))
                            )
                        )
...
export class IdolJsCodegenTypeStruct implements GeneratorContext {
    ...
    get collectionConstructorExpr(): Alt<Expression> {
        const containerAndArgs: Alt<[Expression, {[k: string]: any}]> = this.tsDecon
            ...
            .either(this.tsDecon.getRepeated().map(_ => [importExpr(this.idolJS.idolJsFile.list), this.listConstructorArgs]));
