...
export class HttpServiceBase {
    ...
    _invoke(methodConfig, args) {
        const { url, body, method, headers } = this._getRequestConfig(methodConfig, args);
    ...
    _getRequestConfig({ servicePath, methodPath, pathMappings, method }, args) {
        ...
        methodPath = methodPath.split("/").map(segment => {
            if (segment.startsWith(":")) {
                if (typeof args !== "object") throw new Error("No arguments provided for path parameter " + segment);
