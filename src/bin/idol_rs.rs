...
impl<'a> ModuleBuildEnv<'a> {
    ...
    fn gen_scalar_impls(&mut self, t: &Type, type_struct: &TypeStruct) -> BResult {
        let scalar_type = type_struct
            .display_scalar_type(&self.build_env.root_rust_module, &self.idol_module_name);
    ...
    fn gen_literal_impls(&mut self, t: &Type, type_struct: &TypeStruct) -> BResult {
        ...
        self.write(format!(
            "{}(({}).to_owned())",
            t.named.type_name, literal_wrapper
        ))?;
        self.end_block()?;
        self.end_block()?;
        ...
        match type_struct.primitive_type {
            ...
            PrimitiveType::double => {
                self.start_block(format!("if value.is_null() || value.is_f64()"))?
            }
            PrimitiveType::int => {
                self.start_block(format!("if value.is_null() || value.is_i64()"))?
            }
    ...
    fn gen_type_dec(&mut self, t: &Type) -> BResult {
        if let Some(alias) = &t.is_a {
            ...
            if alias.literal.is_some() {
                self.write(format!(
                    "pub struct {}({});",
                    t.named.type_name, display_type
                ))?;
            } else {
                self.write(format!(
                    "pub struct {}(pub {});",
