    ...
    impl ValidatesJson for i64 {
        fn validate_json(value: &serde_json::Value) -> ValidationResult {
            if let serde_json::Value::String(s) = value {
                return s.parse::<i64>().map(|_| ()).map_err(|_| {
                    ValidationError("value was not a properly formatted i64.".to_string())
                });
            }
        ...
        fn validate_json(value: &serde_json::Value) -> ValidationResult {
            if let serde_json::Value::Array(contents) = value {
                for (i, element) in contents.iter().enumerate() {
                    T::validate_json(element)
                        .map_err(|s| ValidationError(format!("index {}: {}", i, s)))?;
                }
        ...
        fn validate_json(value: &serde_json::Value) -> ValidationResult {
            if let serde_json::Value::Object(contents) = value {
                for entry in contents.iter() {
                    T::validate_json(entry.1)
                        .map_err(|s| ValidationError(format!("field {}: {}", entry.0, s)))?;
                }
